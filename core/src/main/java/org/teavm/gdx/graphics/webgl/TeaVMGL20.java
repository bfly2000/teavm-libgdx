...
public class TeaVMGL20 implements GL20 {
	...
	@JSBody(params = "context", script = "if (typeof(WebGLDebugUtils) == 'undefined') {" + "return context;" + "}"
	...
	public Float32Array copy (FloatBuffer buffer) {
		...
		final Float32Array result = Float32Array.create(buffer.remaining());
		float[] tmp;
		if (buffer.hasArray()) {
			tmp = buffer.array();
		} else {
	...
	public Float64Array copy (DoubleBuffer buffer) {
		buffer = buffer.duplicate();
		final Float64Array result = Float64Array.create(buffer.remaining());
		double[] tmp;
		if (buffer.hasArray()) {
			tmp = buffer.array();
		} else {
	...
	public Int16Array copy (ShortBuffer buffer) {
		buffer = buffer.duplicate();
		final Int16Array result = Int16Array.create(buffer.remaining());
		short[] tmp;
		if (buffer.hasArray()) {
			tmp = buffer.array();
		} else {
	...
	public Int32Array copy (IntBuffer buffer) {
		buffer = buffer.duplicate();
		final Int32Array result = Int32Array.create(buffer.remaining());
		int[] tmp;
		if (buffer.hasArray()) {
			tmp = buffer.array();
		} else {
	...
	public Int8Array copy (ByteBuffer buffer) {
		buffer = buffer.duplicate();
		final Int8Array result = Int8Array.create(buffer.remaining());
		byte[] tmp;
		if (buffer.hasArray()) {
			tmp = buffer.array();
		} else {
	...
	public Uint8Array copyU (ByteBuffer buffer) {
		buffer = buffer.duplicate();
		final Uint8Array result = Uint8Array.create(buffer.remaining());
		byte[] tmp;
		if (buffer.hasArray()) {
			tmp = buffer.array();
		} else {
	...
	public void glReadPixels (final int x, final int y, final int width, final int height, final int format, final int type,
		...
		final Uint8Array buffer = Uint8Array.create(size);
		...
		for (int i = 0; i < size; i++) {
			pixelsByte.put((byte)buffer.get(i));
		}
	}
